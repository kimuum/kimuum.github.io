$(function () {
  ...
  $(window).scroll(function () {
    ...
    if ($wrapper.hasClass('home') || $wrapper.hasClass('menu-02')) {
  ...
  // 메인배너 다음 섹션으로 스크롤
  var $nextSectionBtn = $('.btn-next-section');
  $nextSectionBtn.on('click', function () {
    var $nextSection = $('.info-container');
  ...
  // App Service
  var swiperAppServiceImage = new Swiper(".image-container", {
    slidesPerView: "auto",
    centeredSlides: true,
    navigation: {
      nextEl: ".swiper-button-next",
    ...
    on: {
      slideChange: function () {
        active_Index = swiperAppServiceImage.realIndex;
        swiperAppServiceInfo.slideTo(active_Index);
      },
    }    
  });

  var swiperAppServiceInfo = new Swiper(".info-container", {
    slidesPerView: 1,
    breakpoints: {
      // when window width is >= 1024px
      1024: {
    ...
    on: {
      slideChange: function () {
        active_Index = swiperAppServiceInfo.realIndex;
        swiperAppServiceImage.slideTo(active_Index);
      },
    }    
  });
