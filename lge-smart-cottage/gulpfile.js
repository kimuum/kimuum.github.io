...
// Admin JS function
function adminJs() {
  const source = './src/js/admin/*.js';

  return src(source).pipe(changed(source)).pipe(concat('adminBundle.js')).pipe(dest('./dist/assets/js/')).pipe(browsersync.stream());
}

// CSS function
function css() {
  const source = ['./src/scss/style.scss', '!./src/scss/general.scss', '!./src/scss/admin.scss'];
...
// Admin CSS function
function adminCss() {
  const source = ['./src/scss/admin.scss', '!./src/scss/general.scss', '!./src/scss/style.scss'];
...
function watchFiles() {
  ...
  watch('./src/scss/**', parallel(css, generalCss, adminCss));
  watch('./src/js/**', series(js, lib_js, adminJs));
...
exports.build = parallel(html, lib_js, js, adminJs, css, generalCss, adminCss, font, images, codingListCss);
