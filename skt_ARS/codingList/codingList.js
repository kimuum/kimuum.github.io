const allData = [dataPage, dataPage3];
    ...
    data.forEach((cell, i) => {
      if (cell.children) {
        ...
        fileLink.setAttributes({
          href: 'dev/' + cell.link + cell.name,
          target: '_blank',
        });

        if (cell.content) row.push({ content: cell.content });
        if (cell.name && cell.link) row.push({ content: fileLink });
        if (cell.description) row.push({ content: cell.description });
        row.push({ content: cell.update });
        row.push({ content: cell.etc });
