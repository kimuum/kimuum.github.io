...
(function ($) {
  $.alerts = {
    ...
    check: function (message, title, callback) {
      if (title == null) title = 'Check';
      // 버튼 text 세팅.
      $.alerts.okButton = '확인';

      $.alerts._show(title, message, null, 'check', function (result) {
        if (callback) callback(result);
      });
    },
  ...
  jCheck = function (message, title, callback) {
    $.alerts.check(message, title, callback);
  };
})(jQuery);
